...
class MonoNeRFTrainer(trainlib.Trainer):
    ...
    def train_step(self, data, global_step):
        ...
        quarter = self.conf.get_int("model.training_frames", 3)
        ...
        if self.rank == 0:
            ...
            self.logger.warning('Use QUARTER frames for training')


        decay_iteration = self.conf.get("info.decay_iteration", max(25, num_img))

        # Pre-train StaticNeRF
        if self.args.pretrain and self.conf.get_bool("info.use_unseen_frame_pretrain", True):
            ...
            with logging_redirect_tqdm():
                for i in tqdm(range(self.args.N_static_iters), mininterval=10):
                    ...
                    for b in range(images.shape[0]):
                        ...
                        # img_i = np.random.choice(i_train[:quarter])
                        # We use all images for training static background for better quality
                        # Or we could just use training frames to train the static background
                        img_i = np.random.choice(i_train)
                        curr_t = img_i / num_img * 2. - 1.0 # time of the current frame ## range from -1 to 1
                        if self.conf.get_bool("info.without_time", False):
                            curr_t = 0
        ...
        with logging_redirect_tqdm():
            for i in tqdm(range(self.start, self.args.N_iters), mininterval=10):
                ...
                # Compute MSE loss between rgb_s and true RGB.
                if self.conf.get_bool("info.use_unseen_frame_pretrain", True):
                ...
                if self.conf.get_bool("info.use_unseen_frame_pretrain", True):
                    ...
                    loss += self.args.full_loss_lambda * loss_dict['img_d_loss']
